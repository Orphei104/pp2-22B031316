...
pygame.display.set_caption('UDAV LUTUYI')
...
fruit_spawn = True
'''
big_fruit_position = [random.randrange(1,(window_x // 15)) * 40,
                      random.randrange(1, (window_y // 15) * 40)]
...
# Отображаем функцию Level of da snake
def show_level(choice, color , font ,size) :
    ...
    # создать объект поверхности отображения
    # score_surface
    level_surface = level_font.render('Level : ' + str(level), True, white)
...
while True :
    ...
    #Создаем большой фрукт который будет держаться некоторое время
    '''snake_body.insert(0, list(snake_position))
    if snake_position[0] == big_fruit_position[0] and snake_position[1] == big_fruit_position[1] :
        score += 20
        fruit_spawn = False
    else :
        snake_body.pop()
    ...
    if not big_fruit_spawn :
        big_fruit_position = [random.randrange(1, (window_x // 10)) * 10,
                              random.randrange(1, (window_y // 10)) * 10]
    big_fruit_spawn = True
    game_window.fill(black)
    ...
    pygame.draw.rect(game_window, red, pygame.Rect(
        big_fruit_position[0], big_fruit_position[1],25 ,25))'''
...
''' TO write a code for a big fruit and to apply the timer and size 
of course use a new level adder'''